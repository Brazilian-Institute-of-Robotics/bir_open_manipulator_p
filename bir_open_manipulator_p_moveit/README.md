...
Package developed to do **OMPL planners benchmark** for OpenManipulator-PRO using MoveIt!
...
It's necessary to get those dependencies in your workspace:
...
Here, you will use RViz to create your scenes, queries and start states. The steps here was finished before, **so it is only necessary if you want to create your own tests**, if it is not the case go to **section 1.2.**
...
```
Where **<diretorio_do_log>** is the path (where the created log is, defined in **bir_omp_benchmark_settings.yaml**) for one of your logs created after the benchmark. Every query generate one log file.
